...
_KMHFL_PROD_API_URL: Final[str] = "https://api.kmhfl.health.go.ke"
...
@define
class KMHFLFacilityLoader(Loader[Iterable[Facility]]):
    _kmhfl_api_url: str = field(  # pyright: ignore[reportCallIssue]
        alias="kmhfl_api_url",
    ...
    def of_file_path(cls, _: str) -> Self:
        ...
        # The function is expected to take a file path of the metadata tp
    ...
    def _load_facilities_on_page(
        ...
        res: Response = self._make_request(
            req=Request(
                ...
                # -> url=f"{self._kmhfl_api_url}/api/facilities/facilities/",
                url=f"{self._kmhfl_api_url}/api/facilities/material/",
            ),
        )
    ...
    def _authenticate(self) -> AuthBase:
        res: Response = self._session.post(
            ...
            url=f"{self._kmhfl_api_url}/o/token/",
        )
...
class _Authenticated(AuthBase):
    """
    The ``Auth`` implementation used by the :class:`KMHFLFacilityLoader`
